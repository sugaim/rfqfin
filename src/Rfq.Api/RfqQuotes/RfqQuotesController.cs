    ...
    BulkWithdrawQuotes bulkWithdraw) : ControllerBase
{
    ...
    public async Task<ConfirmQuoteResponse> Confirm(long caseId, ConfirmQuoteRequest request,
        CancellationToken cancellationToken) => RfqQuotesApiMapper.ToApi(
        await confirm.ExecuteAsync(new CaseId(caseId), QuoteApiMapper.ToDomain(request.Expiry),
...
public sealed record LifecycleItemRequest([Range(1, long.MaxValue)] long CaseId,
    [Range(1, long.MaxValue)] long ExpectedCurrentVersion);
...
public sealed record ConfirmQuoteResponse(long CaseId, Guid QuoteId, Guid RevisionId,
    RfqStatusValue RfqStatus, QuoteStatusValue QuoteStatus, QuoteMode Mode,
    CalculatedQuoteResponse? Calculated, ManualQuoteResponse? Manual,
    DateTimeOffset ConfirmedAt, QuoteExpiryResponse Expiry,
    DateTimeOffset? ExpiresAt, long CurrentVersion);
...
public static class RfqQuotesApiMapper
{
    public static ConfirmQuoteResponse ToApi(ConfirmQuoteResult value) => new(
        value.CaseId.Value, value.QuoteId.Value, value.RevisionId.Value,
        Map<RfqStatusValue>(value.RfqStatus), Map<QuoteStatusValue>(value.QuoteStatus),
        QuoteApiMapper.ToApi(value.Mode), QuoteApiMapper.ToApi(value.Calculated),
        QuoteApiMapper.ToApi(value.Manual), value.ConfirmedAt,
        QuoteApiMapper.ToApi(value.Expiry), value.ExpiresAt, value.CurrentVersion.Value);
    public static LifecycleResponse ToApi(LifecycleResult value) => new(
        value.CaseId.Value, Map<RfqStatusValue>(value.RfqStatus),
