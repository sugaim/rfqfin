    ...
    BulkDiscardInitialDrafts bulkDiscard) : ControllerBase
{
    ...
    public async Task<InitialRfqResponse> Confirm(
        long caseId,
        UpdateDraftRequest request,
        CancellationToken cancellationToken) => RfqDraftsApiMapper.ToApi(
    ...
    public async Task<IActionResult> Discard(
        ...
        CancellationToken cancellationToken)
    {
        await discardDraft.ExecuteAsync(
            new CaseId(caseId),
    ...
    public async Task<IReadOnlyList<BulkItemResponse>> BulkDiscard(
        ...
        [.. (await bulkDiscard.ExecuteAsync(
            [.. request.Items.Select(item => new DiscardInitialDraftItem(
                new CaseId(item.CaseId), new StateVersion(item.ExpectedVersion)))],
            cancellationToken)).Select(BulkApiMapper.ToApi)];
}
...
public static class RfqDraftsApiMapper
{
    ...
    public static InitialRfqResponse ToApi(InitialRfqResult result) => new(
        ...
        Map<RfqStatusValue>(result.RfqStatus),
        Map<RevisionStatusValue>(result.RevisionStatus),
        MapNullable<QuoteStatusValue>(result.QuoteStatus),
        MapNullable<QuoteRequestReasonValue>(result.QuoteRequestReason),
