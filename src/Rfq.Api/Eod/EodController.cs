...
{
    [HttpGet]
    public async Task<IReadOnlyList<EodResponse>> Get([FromQuery] DateOnly date,
        CancellationToken token) => (await queries.GetEodAsync(date, token))
        .Select(value => new EodResponse(value.ContactOwnerId.Value,
            value.Open, value.Hit, value.Away)).ToArray();
}
