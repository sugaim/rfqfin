...
public sealed class RfqAmendmentsController(
    SaveAmendment save, ConfirmAmendment confirm, DiscardAmendment discard,
    BulkConfirmAmendments bulkConfirm, BulkDiscardAmendments bulkDiscard) : ControllerBase
{
    [HttpPut("{caseId:long}/amendment")]
    public async Task<AmendmentResponse> Save(long caseId, SaveAmendmentRequest request,
        CancellationToken token) => AmendmentApiMapper.ToApi(await save.ExecuteAsync(
            new SaveAmendmentCommand(new CaseId(caseId), request.Notional, request.SettlementDate,
                request.Message, new StateVersion(request.ExpectedCurrentVersion),
                request.ExpectedDraftVersion is null ? null
                    : new StateVersion(request.ExpectedDraftVersion.Value)), token));

    [HttpPost("{caseId:long}/amendment/confirm")]
    public async Task<AmendmentResponse> Confirm(long caseId, AmendmentActionRequest request,
    ...
    public async Task<AmendmentResponse> Discard(long caseId, AmendmentActionRequest request,
...
public sealed record SaveAmendmentRequest(decimal? Notional, DateOnly? SettlementDate,
    string? Message, [Range(1, long.MaxValue)] long ExpectedCurrentVersion,
...
public sealed record AmendmentResponse(long CaseId, Guid CurrentRevisionId,
...
public static class AmendmentApiMapper
{
    public static AmendmentItem ToItem(long caseId, AmendmentActionRequest value) => new(
        new CaseId(caseId), new StateVersion(value.ExpectedCurrentVersion),
        new StateVersion(value.ExpectedDraftVersion));
    public static AmendmentItem ToItem(AmendmentActionItemRequest value) => new(
        new CaseId(value.CaseId), new StateVersion(value.ExpectedCurrentVersion),
        new StateVersion(value.ExpectedDraftVersion));
    public static AmendmentResponse ToApi(AmendmentResult value) => new(
        value.CaseId.Value, value.CurrentRevisionId.Value, value.DraftRevisionId?.Value,
