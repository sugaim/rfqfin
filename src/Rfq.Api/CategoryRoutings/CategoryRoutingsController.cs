...
public sealed class CategoryRoutingsController(ICategoryRouting routings) : ControllerBase
{
    ...
    public async Task<IReadOnlyList<CategoryRoutingResponse>> Get(
        CancellationToken cancellationToken) => [.. (await routings.GetAllAsync(cancellationToken)).Select(CategoryRoutingApiMapper.ToApi)];

    [HttpPut("{categoryId}")]
    public async Task<CategoryRoutingResponse> Put(
        string categoryId,
        UpdateCategoryRoutingRequest request,
...
public sealed record CategoryRoutingResponse(
    string CategoryId,
    string CategoryName,
    string DefaultTraderId,
