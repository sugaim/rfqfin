...
{
    [HttpPut("sales")]
    public async Task<SalesMemoResponse> Sales(long caseId, UpdateMemoRequest request,
        CancellationToken token)
    {
        var value = await updateSales.ExecuteAsync(new CaseId(caseId), request.Memo,
            new StateVersion(request.ExpectedVersion), token);
        return new(value.CaseId.Value, value.Memo, value.Version.Value);
    }

    [HttpPut("trader")]
    public async Task<TraderMemoResponse> Trader(long caseId, UpdateMemoRequest request,
        CancellationToken token)
    {
        var value = await updateTrader.ExecuteAsync(new CaseId(caseId), request.Memo,
            new StateVersion(request.ExpectedVersion), token);
        return new(value.CaseId.Value, value.Memo, value.Version.Value);
    }
}

public sealed record UpdateMemoRequest(string? Memo,
