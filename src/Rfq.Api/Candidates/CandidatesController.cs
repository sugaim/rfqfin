    ...
    ICurrentUser currentUser) : ControllerBase
{
    ...
    public async Task<IReadOnlyList<UserCandidateResponse>> AssignableTraders(
        CancellationToken cancellationToken) => (await users.GetAssignableTradersAsync(
            currentUser.User.DeskId, cancellationToken)).Select(CandidateApiMapper.ToApi).ToArray();
    ...
    public async Task<IReadOnlyList<ClientCandidateResponse>> Clients(
        ...
        (await clients.SearchAsync(q, cancellationToken)).Select(CandidateApiMapper.ToApi).ToArray();

    [HttpGet("api/rfqs/candidates/securities")]
    public async Task<IReadOnlyList<SecurityCandidateResponse>> Securities(
        [FromQuery, Required] string q, CancellationToken cancellationToken) =>
        (await securities.SearchAsync(q, cancellationToken)).Select(CandidateApiMapper.ToApi).ToArray();
}
...
public sealed record SecurityCandidateResponse(string SecurityId, string JapaneseName,
    string BbgDisplay, string InternalCode, string Isin, string CategoryId, string CategoryName);
...
public static class CandidateApiMapper
{
    ...
    public static SecurityCandidateResponse ToApi(SecuritySearchResult value) => new(
        value.SecurityId.Value, value.JapaneseName, value.BbgDisplay, value.InternalCode,
        value.Isin, value.CategoryId.Value, value.CategoryName);
}
