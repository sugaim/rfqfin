...
{
    [HttpGet]
    public async Task<IReadOnlyList<RevisionResponse>> Get(
        long caseId,
        CancellationToken token) => [.. (await revisions.GetAsync(new CaseId(caseId), token)).Select(RevisionApiMapper.ToApi)];
}
...
public static class RevisionApiMapper
{
    public static RevisionResponse ToApi(RevisionHistoryItem value) => new(
        ...
        Enum.Parse<RevisionStatusValue>(value.Status.ToString()),
        value.Notional,
        ...
        value.CreatedAt,
        value.ConfirmedAt,
        value.CopiedFromRevisionId?.Value,
