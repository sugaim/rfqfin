...
public sealed class RfqOwnershipController(
    ...
    ReleaseRfq release,
    AssignTrader assign,
    TakeOverRfq takeOver,
    BulkPickUpRfqs bulkPickUp,
    BulkReleaseRfqs bulkRelease,
    BulkAssignTrader bulkAssign) : ControllerBase
{
    [HttpPost("ownership/pick-up")]
    public async Task<OwnershipResponse> PickUp(
        long caseId,
    ...
    public async Task<OwnershipResponse> Release(
        long caseId,
    ...
    public async Task<OwnershipResponse> TakeOver(
        long caseId,
        ConfirmedRequest request,
    ...
    public async Task<OwnershipResponse> Assign(
        long caseId,
    ...
    public async Task<IReadOnlyList<BulkItemResponse>> BulkPickUp(
        ...
        [.. (await bulkPickUp.ExecuteAsync(
            [.. request.Items.Select(item => new PickUpRfqItem(
                new CaseId(item.CaseId), new StateVersion(item.ExpectedVersion), item.Confirmed))],
            token)).Select(BulkApiMapper.ToApi)];
    ...
    public async Task<IReadOnlyList<BulkItemResponse>> BulkRelease(
        ...
        [.. (await bulkRelease.ExecuteAsync(OwnershipApiMapper.ToItems(request.Items), token)).Select(BulkApiMapper.ToApi)];
    ...
    public async Task<IReadOnlyList<BulkItemResponse>> BulkAssign(
        ...
        [.. (await bulkAssign.ExecuteAsync(
            UserId.Create(request.TargetAssignedTraderId),
            OwnershipApiMapper.ToItems(request.Items),
            token))
...
public sealed record PickUpItemRequest(
    [Range(1, long.MaxValue)] long CaseId,
    [Range(1, long.MaxValue)] long ExpectedVersion,
    bool Confirmed);
...
{
    public static OwnershipResponse ToApi(OwnershipResult value) => new(
        value.CaseId.Value,
    ...
    public static OwnershipBulkItem[] ToItems(IReadOnlyList<OwnershipItemRequest> items) =>
        [.. items.Select(item => new OwnershipBulkItem(
            new CaseId(item.CaseId),
            new StateVersion(item.ExpectedVersion)))];
}
