...
export function patchWorkingQuote(
  row: TraderRfq,
  value: WorkingQuoteResult,
): TraderRfq {
  return {
    ...row,
    workingQuoteMode: value.mode,
    calculated: value.calculated,
    manual: value.manual,
    workingQuoteVersion: value.version,
...
export function patchOwnership(
  row: TraderRfq,
  value: OwnershipResult,
): TraderRfq {
  return {
    ...row,
    assignedTraderId: value.assignedTraderId,
    owned: value.owned,
    currentVersion: value.currentVersion,
  }
}

export function patchContactOwner(
  row: TraderRfq,
  value: ContactOwnerResult,
): TraderRfq {
  return {
    ...row,
    contactOwnerId: value.contactOwnerId,
    currentVersion: value.currentVersion,
  }
}
...
export function patchLifecycle(
  ...
  return {
    ...
    rfqStatus: value.rfqStatus,
    quoteStatus: value.quoteStatus,
    quoteRequestReason: value.quoteRequestReason,
    currentVersion: value.currentVersion,
  }
}

export function patchConfirmedQuote(
  row: TraderRfq,
  value: ConfirmQuoteResult,
): TraderRfq {
  return {
    ...row,
