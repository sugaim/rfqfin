...
{
    public async Task<IReadOnlyList<RevisionHistoryItem>> GetAsync(
        CaseId caseId,
        CancellationToken cancellationToken = default) =>
        await dbContext.RfqRevisions.AsNoTracking().Where(item => item.CaseId == caseId.Value)
            .OrderByDescending(item => item.CreatedAt)
            .Select(item => new RevisionHistoryItem(
                new RevisionId(item.RevisionId),
                item.Status,
                item.Notional,
                item.SettlementDate,
                item.SalesAndTradingMessage,
                new StateVersion(item.Version),
                item.CreatedAt,
                item.ConfirmedAt,
