    ...
    ICurrentUser currentUser) : IRfqSearchQueries
{
    ...
    {
        IQueryable<RfqCaseEntity> query = dbContext.RfqCases.AsNoTracking()
        ...
        if (search.CreatedFrom is not null || search.CreatedTo is not null)
        {
            ...
            {
                DateTimeOffset from = DeskDateBoundary.ToUtc(search.CreatedFrom.Value, deskTimeZone);
                query = query.Where(item => item.CreatedAt >= from);
            }
            if (search.CreatedTo is not null)
            {
                DateTimeOffset to = DeskDateBoundary.ToUtc(search.CreatedTo.Value.AddDays(1), deskTimeZone);
                query = query.Where(item => item.CreatedAt < to);
            }
        }
        if (search.ClientId is not null)
        {
            query = query.Where(item => item.ClientId == search.ClientId.Value);
        }

        if (search.SecurityId is not null)
        {
            query = query.Where(item => item.SecurityId == search.SecurityId.Value);
        }

        if (search.CategoryId is not null)
        {
            query = query.Where(item => item.CategorySnapshot == search.CategoryId.Value);
        }

        if (search.ContactOwnerId is not null)
        {
            query = query.Where(item => item.Current.ContactOwnerId == search.ContactOwnerId.Value);
        }

        if (search.SalesId is not null)
        {
            query = query.Where(item => item.SalesId == search.SalesId.Value);
        }

        if (search.AssignedTraderId is not null)
        {
            query = query.Where(item => item.Current.AssignedTraderId == search.AssignedTraderId.Value);
        }

        if (search.Status is not null)
        {
            query = query.Where(item => item.Current.RfqStatus == search.Status.Value);
        }

        if (search.CaseId is not null)
        {
        ...
        List<RfqSearchItem> rows = await query.OrderByDescending(item => item.CreatedAt).Take(ResultCap + 1)
            .Select(item => new RfqSearchItem(
                ...
                CategoryId.Create(item.CategorySnapshot),
                item.Current.RfqStatus,
                ...
                item.Current.CurrentRevision.Notional,
                item.Current.CurrentRevision.SettlementDate,
                null,
                null,
                null))
        ...
        List<SearchQuoteRef> quoteRefs = await dbContext.RfqCases.AsNoTracking()
            ...
            .Select(item => new SearchQuoteRef(
                item.CaseId,
                item.Current.CurrentRevisionId,
                item.Current.CurrentQuoteId ?? item.Current.ClosedQuoteId))
            .ToListAsync(cancellationToken);
        Guid[] revisionIds = [.. quoteRefs.Select(item => item.RevisionId).Distinct()];
        Guid[] quoteIds = [.. quoteRefs.Where(item => item.QuoteId != null)
            .Select(item => item.QuoteId!.Value).Distinct()];
        Dictionary<Guid, WorkingQuoteEntity> workingQuotes = await dbContext.WorkingQuotes.AsNoTracking()
        ...
        RfqSearchItem[] projected = [.. cappedRows.Select(row =>
        {
            SearchQuoteRef reference = quoteRefsByCase[row.CaseId.Value];
            QuoteSummary summary;
            if (reference.QuoteId is { } quoteId
                && confirmedQuotes.TryGetValue(quoteId, out ConfirmedQuoteEntity? confirmed))
            {
                summary = ToSummary(
                    confirmed.Mode,
                    confirmed.CalculatedPayloadJson,
                    confirmed.ManualPayloadJson);
            }
            else if (workingQuotes.TryGetValue(reference.RevisionId, out WorkingQuoteEntity? working))
            {
                summary = ToSummary(
                    working.Mode,
                    working.CalculatedPayloadJson,
        ...
        string? manualJson)
    {
        ...
        {
            ManualQuotePayload manual = QuotePayloadPersistence.DeserializeManual(manualJson
        ...
        CalculatedQuotePayload calculated = QuotePayloadPersistence.DeserializeCalculated(calculatedJson);
