...
public static class WorkingQuoteTransitions
{
    public static WorkingQuote ApplyCalculated(
        WorkingQuote quote, CalculatedQuotePayload payload,
        StateVersion expectedVersion, UserId updatedBy, DateTimeOffset updatedAt)
    {
    ...
    public static WorkingQuote SwitchMode(
        WorkingQuote quote, WorkingQuoteMode mode,
        StateVersion expectedVersion, UserId updatedBy, DateTimeOffset updatedAt)
    {
        EnsureVersion(quote, expectedVersion);
        var manual = mode == WorkingQuoteMode.Manual && quote.Mode != WorkingQuoteMode.Manual
    ...
    public static WorkingQuote UpdateManual(
        WorkingQuote quote, decimal? price, decimal? finalSimpleYield,
        StateVersion expectedVersion, UserId updatedBy, DateTimeOffset updatedAt)
    {
        EnsureVersion(quote, expectedVersion);
        ...
        return Copy(quote, quote.Mode, quote.Calculated,
            new ManualQuotePayload(price, finalSimpleYield), updatedBy, updatedAt);
    }
    ...
    private static WorkingQuote Copy(
        ...
        UserId updatedBy, DateTimeOffset updatedAt) => new(
            quote.RevisionId, mode, calculated, manual, quote.Version.Next(),
            quote.CreatedAt, quote.CreatedBy, updatedAt, updatedBy);
}
