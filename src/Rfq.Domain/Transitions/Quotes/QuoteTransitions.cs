...
public static class QuoteTransitions
{
        ...
        QuoteConfirmation confirmation)
    {
        ...
        DateOnly settlementDate = rfq.CurrentRevision.SettlementDate
        ...
        (int? minutes, DateTimeOffset? expiresAt) = confirmation.Expiry switch
        {
        ...
        var quote = new ConfirmedQuote(
            ...
            expiresAt,
            requested.Reason);
        RfqCase next = rfq.Next(lifecycle: new ActiveRfq(
    ...
    {
        rfq.EnsureVersion(expectedVersion);
        OpenRfq open = rfq.Lifecycle as OpenRfq
            ?? throw new DomainRuleViolationException("Only an Open quote can expire.");
        QuoteId? current = rfq.CurrentQuoteId;
        if (current != quoteId)
        {
