...
public sealed class LifecycleContractTests
{
    ...
    [InlineData(nameof(RfqLifecycleController.CorrectToHit),
        "{caseId:long}/outcome/correct-to-hit")]
    [InlineData(nameof(RfqLifecycleController.CorrectToAway),
    ...
    {
        var method = typeof(RfqLifecycleController).GetMethod(methodName)!;
        Assert.Equal(route, method.GetCustomAttributes(typeof(HttpPostAttribute), false)
    ...
    public void Application_close_and_correction_commands_do_not_accept_rfq_status()
    {
        ...
        {
            typeof(CloseHitRfq), typeof(CloseAwayRfq),
            typeof(CorrectOutcomeToHit), typeof(CorrectOutcomeToAway),
        })
        {
            var method = Assert.Single(type.GetMethods(), value => value.Name == "ExecuteAsync");
            Assert.DoesNotContain(method.GetParameters(), parameter =>
                parameter.ParameterType == typeof(RfqStatus));
        }
    }
