...
public sealed class CaseOperationTests
{
    public static TheoryData<ExpectedRfqException, CaseOperationFailureCode> SupportedErrors => new()
    {
        { new RfqRequestValidationException("validation"), CaseOperationFailureCode.Validation },
        { new DomainRuleViolationException("state"), CaseOperationFailureCode.InvalidState },
        { new StateVersionMismatchException("version"), CaseOperationFailureCode.VersionConflict },
        { new RfqNotFoundException("missing"), CaseOperationFailureCode.NotFound },
        { new RfqForbiddenException("forbidden"), CaseOperationFailureCode.Forbidden },
    };
    ...
    public async Task Successful_items_commit_independently()
    {
        ...
        IReadOnlyList<CaseOperationResult> results = await CaseOperation.ExecuteAsync(
        ...
        Assert.All(results, result => Assert.Equal(CaseOperationStatus.Applied, result.Status));
    ...
    public async Task Mixed_results_continue_after_recoverable_failure_and_discard_state()
    {
        ...
        IReadOnlyList<CaseOperationResult> results = await CaseOperation.ExecuteAsync(
            ...
            {
                1 => Task.FromResult(CaseOperationOutcome.Applied),
                2 => Task.FromResult(CaseOperationOutcome.NoChange),
                3 => Task.FromException<CaseOperationOutcome>(
                    new StateVersionMismatchException("changed")),
                _ => Task.FromResult(CaseOperationOutcome.Applied),
            },
            unitOfWork,
            CancellationToken.None);

        Assert.Equal(
            [CaseOperationStatus.Applied, CaseOperationStatus.NoChange,
                CaseOperationStatus.Failed, CaseOperationStatus.Applied],
            results.Select(result => result.Status));
        Assert.Equal(CaseOperationFailureCode.VersionConflict, results[2].FailureCode);
    ...
    public async Task Supported_error_kind_is_failed_discarded_and_next_item_runs(
        ...
        CaseOperationFailureCode expectedCode)
    {
        var unitOfWork = new UnitOfWork();
        ...
        IReadOnlyList<CaseOperationResult> results = await CaseOperation.ExecuteAsync(
            ...
            (item, _) =>
            {
                ...
                return item.Value == 1
                    ? Task.FromException<CaseOperationOutcome>(exception)
                    : Task.FromResult(CaseOperationOutcome.Applied);
            },
            unitOfWork,
            CancellationToken.None);
        ...
        Assert.Equal(expectedCode, results[0].FailureCode);
    ...
    public async Task Request_cancellation_aborts_bulk()
    {
        ...
        cancellation.Cancel();
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() => CaseOperation.ExecuteAsync(
            [new CaseId(1)],
            item => item,
            (_, _) => Task.FromResult(CaseOperationOutcome.Applied),
            new UnitOfWork(),
            cancellation.Token));
    }
